...
function buildPromptInput(
  ...
  return {
    ...
    hardRules: [
      ...
      "Quantity applies to the complete named product, even when model tokens contain digits. For example, 'patru Samsung Tab S7 Plus' means quantity 4, not one S7 device.",
      "Multiply item mass and volume for every unit. When each unit has its own box, add packaging for every unit; add one outer shipping package only when the description or image implies it.",
      "Text read from an image such as 'net weight', 'greutate netă' or a standalone product mass is a firm content mass: preserve it and add only an adaptive packaging margin. Text explicitly marked 'gross weight' or 'greutate brută' already includes packaging, so do not add another margin.",
      "If OCR digits, quantity or units are ambiguous or conflict, lower confidence and ask a targeted clarification instead of silently choosing the smaller value.",
