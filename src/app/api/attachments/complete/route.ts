...
const schema = z.object({
  ...
  objectKey: z.string().trim().min(1).max(1024).regex(/^[A-Za-z0-9/_\-.]+$/u),
...
export async function POST(request: Request) {
  ...
  try { return NextResponse.json({ attachment: await completeAttachmentUpload(identity, parsed.data) }); }
    ...
    const reason = publicErrorCode(
      error,
      ["forbidden", "ticket_read_only", "evaluation_read_only", "message_not_found", "ticket_closed", "evaluation_closed", "invalid_image", "invalid_object_key", "attachment_limit", "attachment_exists"] as const,
      "upload_unavailable",
    );
