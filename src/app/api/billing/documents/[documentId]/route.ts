...
export async function GET(_request: Request, context: { params: Promise<{ documentId: string }> }) {
  ...
  const { data } = await database.from("billing_documents")
    .select("id,pdf_object_key,generation_status,order:orders!inner(sender_profile_id)")
  ...
  return NextResponse.redirect(await createR2DownloadUrl(data.pdf_object_key), 302);
}
